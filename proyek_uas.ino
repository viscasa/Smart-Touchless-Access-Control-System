...
// GANTI DENGAN WIFI HP ANDA (2.4 GHz)
const char* ssid       = "Xiaomi 13T";     
const char* password   = "xiaomay99"; 
...
// MAC Address Target: 88:57:21:94:69:70
  ...
  for (;;) {
    if (xQueueReceive(accessQueue, &receivedCommand, portMAX_DELAY) == pdPASS) {
      if (receivedCommand == 1) {
        ...
        esp_now_send(broadcastAddress, (uint8_t *) &myData, sizeof(myData));
        printDashboardLog(); // <--- LOG SAAT TUTUP (BARU DITAMBAHKAN)
...
if(WiFi.status() == WL_CONNECTED){
    ...
    // --- TAMBAHKAN INI UNTUK MELIHAT CHANNEL ---
